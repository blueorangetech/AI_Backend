...
def group_data(arguments):
    ...
    try:
        ...
        if isinstance(sum_fields, str):
            merge_data["증감 수치"] = merge_data[f"{sum_fields}_x"] - merge_data[f"{sum_fields}_y"]
            merge_data["증감률"] = (merge_data[f"{sum_fields}_x"] / merge_data[f"{sum_fields}_y"] - 1) * 100

        else:
            merge_data["증감 수치"] = merge_data[f"{sum_fields[0]}_x"] - merge_data[f"{sum_fields[0]}_y"]
            merge_data["증감률"] = (merge_data[f"{sum_fields[0]}_x"] / merge_data[f"{sum_fields[0]}_y"] - 1) * 100

        merge_data.sort_values(by="증감 수치", ascending = False, inplace = True)

        # 필드 이름 수정 및 정리
        if isinstance(sum_fields, str):
            merge_data.rename(columns={f"{sum_fields}_x": "분석 기간", f"{sum_fields}_y": "비교 기간"}, inplace = True)
            
        else:
            merge_data.rename(columns={f"{sum_fields[0]}_x": "분석 기간", f"{sum_fields[0]}_y": "비교 기간"}, inplace = True)
