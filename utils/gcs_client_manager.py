...
class GCSClientManager:
    ...
    @classmethod
    async def get_client(cls, config: dict) -> storage.Client:
        """
        설정 기반으로 GCS 클라이언트 반환
        ...
        if config_key not in cls._instances:
            async with cls._lock:
                if config_key not in cls._instances:
                    ...
                    logger.info(f"새로운 GCS 클라이언트가 생성되었습니다. (프로젝트: {credentials.project_id})")
        ...
        if cls._instances:
            async with cls._lock:
                for config_key, client in cls._instances.items():
                    try:
                        client.close()
                        logger.info(f"GCS 클라이언트가 정리되었습니다: {config_key}")
                    except Exception as e:
                        logger.warning(f"GCS 클라이언트 정리 중 오류: {e}")

                cls._instances.clear()
                logger.info("모든 GCS 클라이언트가 정리되었습니다.")
            ...
            async with cls._lock:
                if config_key in cls._instances:
                    try:
                        ...
                        logger.info(f"GCS 클라이언트가 정리되었습니다")
                    except Exception as e:
                        logger.warning(f"GCS 클라이언트 정리 중 오류: {e}")
...
# 전역 함수들
async def get_gcs_client(config: dict) -> storage.Client:
    """전역 GCS 클라이언트 반환"""
    return await GCSClientManager.get_client(config)


async def cleanup_all_gcs_clients():
    """모든 GCS 클라이언트 정리"""
    await GCSClientManager.close_all()
