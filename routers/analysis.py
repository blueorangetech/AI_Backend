...
async def data_analysis(file: UploadFile = File(...), standard: str = Form(), 
    ...
    try:
        ...
        fields, depth, product = json.loads(formula), json.loads(depth), json.loads(product)

        # 필드명 검증과 기간 분리
        pre_result, kst_standard, kst_compare = date_group(file_path, standard, compare, fields + depth)
        
        results = {}

        total_report = analyst(pre_result, kst_standard, kst_compare, fields)
        media_report = media_analyst(pre_result, kst_standard, kst_compare, fields, depth, product)
